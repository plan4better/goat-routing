...
DROP FUNCTION IF EXISTS basic.fill_polygon_h3_10;
CREATE OR REPLACE FUNCTION basic.fill_polygon_h3_10(geom geometry)
...
BEGIN
    ...
    h3_ids AS
    (
        SELECT h3_lat_lng_to_cell(b.geom, 10) h3_index
        FROM border_points b
        UNION ALL
        SELECT h3_polygon_to_cells(p.geom, ARRAY[]::polygon[], 10) h3_index
        FROM polygons p
    )
    SELECT sub.h3_index, basic.to_short_h3_10(sub.h3_index::bigint) AS h3_short,
